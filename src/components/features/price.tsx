...
const PlanCard: React.FC<{ plan: Plan }> = ({ plan }) => {
  ...
  const badge = plan.highlighted ? (
    <div className="absolute -top-3 left-1/2 -translate-x-1/2">
      <span className="inline-flex items-center gap-1 rounded-lg bg-blue-600 px-3 py-1 text-xs font-semibold text-white shadow-sm">
  ...
  return (
    <div className={`relative rounded-2xl bg-white p-4 sm:p-6 md:p-8 shadow-sm ${cardRing}`}> 
      ...
      <div className="mb-6 sm:mb-8 flex items-baseline gap-2">
        <span className={`text-3xl sm:text-4xl font-bold ${priceColor}`}>{plan.price}</span>
        <span className="text-gray-500 text-base sm:text-lg">{plan.per}</span>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="md:hidden mb-8">
          <div className="relative overflow-hidden mt-6 sm:mt-8">
            ...
            >
              {plans.map((plan) => (
                <div key={plan.name} className="w-full flex-shrink-0 px-3 sm:px-4">
                  <PlanCard plan={plan} />
                </div>
              ))}
            </div>
