    ...
    <div className="fixed bottom-0 left-0 right-0 z-[100] p-4 sm:p-6 pb-[max(1rem,env(safe-area-inset-bottom,0px)+0.75rem)]">
      <div className="max-w-4xl mx-auto bg-white rounded-2xl shadow-2xl border border-gray-100 px-5 py-4 sm:px-8 sm:py-5 flex flex-col sm:flex-row items-start sm:items-center gap-4">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-gray-800 font-semibold text-sm sm:text-base mb-0.5">{t('cookies.title')}</p>
        ...
        <div className="flex gap-2 flex-shrink-0 w-full sm:w-auto">
          <button onClick={decline} className="flex-1 sm:flex-none text-sm font-medium px-4 py-2 rounded-xl border border-gray-200 text-gray-600 hover:bg-gray-50 transition-colors">
            {t('cookies.essentialOnly')}
          </button>
          <button onClick={accept} className="flex-1 sm:flex-none text-sm font-semibold px-5 py-2 rounded-xl text-white transition-opacity hover:opacity-90"
            style={{ background: 'linear-gradient(135deg, #2633E1 0%, #26C29E 100%)' }}>
            {t('cookies.acceptAll')}
          </button>
        </div>
